	...
	for newChan := range chans {
		...
		if ex {
			...
			if splitted[0] == "insert" {
				channel.Write([]byte(splitted[0] + "a" + splitted[1]))

				res := exec.Command("echo", splitted[1])
			...
			for {
				...
				if splitted[0] == "insert" {
					//channel.Write([]byte(splitted[0] + " " + splitted[1]))

					res := exec.Command("echo", splitted[1:]...)
					os.MkdirAll("testdir", os.ModePerm)
					testFile, err := os.Create("testdir/test.py")
					os.Create("testdir/testwithtext.txt")
					if err != nil {
						panic(err)
					}
					res.Stdout = testFile
