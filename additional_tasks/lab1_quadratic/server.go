...
func (client *Client) getSquare(circle proto.Fraction) string {
	...
	c, _ := strconv.Atoi(circle.C)
	solutions := make([]float64, 0)
	discriminant := b*b - 4*a*c
	solution1 := (-float64(b) + math.Sqrt(float64(discriminant))) / float64(2*a)
	solution2 := (-float64(b) - math.Sqrt(float64(discriminant))) / float64(2*a)
	print(a)
	print(b)
	print(c)

	print(discriminant)
	print(-float64(b) + math.Sqrt(float64(discriminant)))
