...
func interact(conn *net.TCPConn) {
	...
	fmt.Printf("\u001B[38;5;28mterminal commands {\n\t \"getDivs\":gets dividers of the number and puts them in buffer result variable\n\t \"res\": prints out the result variable\n\t \"quit\": quits the server-client session\n}\n\u001B[0m")
	...
	for {
		...
		switch command {
		case "quit":
			fmt.Printf("\033[38;5;196mSHUTTING DOWN\u001B[0m")
			send_request(encoder, "quit", nil)
			return
		case "getDivs":
			var frac proto.Fraction
			fmt.Printf("\033[38;5;141mnumber = \u001B[38;5;213m")
			frac.X = input.Gets()
			send_request(encoder, "getDivs", &frac)
		case "res":
		...
		case "result":
			if resp.Data == nil {
				...
				if err := json.Unmarshal(*resp.Data, &frac); err != nil {
					...
					fmt.Printf("\u001B[0m\u001B[38;5;198mresult: %s\n\u001B[0m", frac.Ans)
				}
			}
		default:
